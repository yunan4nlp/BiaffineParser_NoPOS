...
def train(data, dev_data, parser, vocab, config):
    ...
    for iter in range(config.train_iters):
        ...
        for onebatch in data_iter(data, config.train_batch_size, True):
            words, extwords, heads, rels, lengths, masks = \
                batch_data_variable(onebatch, vocab)
            parser.model.train()

            parser.forward(words, extwords, masks)
            ...
            uas = np.float64(overall_arc_correct) * 100.0 / np.float64(overall_total_arcs)
            las = np.float64(overall_label_correct) * 100.0 / np.float64(overall_total_arcs)
...
def evaluate(data, parser, vocab, outputFile):
    ...
    for onebatch in data_iter(data, config.test_batch_size, False):
        words, extwords, heads, rels, lengths, masks = \
            batch_data_variable(onebatch, vocab)
        count = 0
        arcs_batch, rels_batch = parser.parse(words, extwords, lengths, masks)
    ...
    uas = np.float64(arc_correct_test) * 100.0 / np.float64(arc_total_test)
    las = np.float64(rel_correct_test) * 100.0 / np.float64(rel_total_test)
